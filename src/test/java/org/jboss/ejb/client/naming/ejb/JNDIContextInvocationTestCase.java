...
public class JNDIContextInvocationTestCase {
    ...
    public void testProxySerializationInvocation() throws Exception {
        ...
        try {
            ...
            // emmartins
            final HttpEJBReceiver httpEJBReceiver = new HttpEJBReceiver("bla bla");
            try {
    ...
    public void testHttp() throws Exception {
        ...
        try {
            ...
            logger.warn("$$$$$$$$$$ EMMARTINS");
            // emmartins
            final HttpEJBReceiver httpEJBReceiver = new HttpEJBReceiver("bla bla");
            httpEJBReceiver.registerModule2(APP_NAME, MODULE_NAME, DISTINCT_NAME);
            try {
                EJBClientContext.requireCurrent().registerEJBReceiver(httpEJBReceiver);
                // now invoke on the serialized proxy. It's expected that the invocation will now use the
                // receiver that we added to the "current" EJB client context
